<script>
	...
	// Get app service URL for callback (full URL needed since auth is on wallet service)
	function getAppServiceUrl() {
		if (!browser) return '';
		const appDomain = env.PUBLIC_DOMAIN_APP || 'localhost:4202';
	...
	async function signInWithGoogle() {
		...
		try {
			...
			const callbackURL = `${appUrl}${redirectPath}`;
